...
{   
    ...
    .alarm_count = (1000000 * m_calculated_interval - 1),
...
void ext_int_init(dimmertyp *ptr)
{
	...
	if (!alreadyInit)
	{
		...
		gpio_set_intr_type(dimZCPin[ptr->current_num], GPIO_INTR_POSEDGE);//GPIO_INTR_NEGEDGE prevous
...
//QueueHandle_t zerocross_evt = NULL;
/*ISR debug region*/
...
//xQueueSendFromISR(zerocross_evt, &gpio_num, NULL);
#if DEBUG_ZERO_CROSS_SIGNAL == ISR_DEBUG_ON
	uint32_t gpio_num = (uint32_t)arg;
